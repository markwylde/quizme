...
- Add **`interrogate`**, a Go CLI that takes a questionnaire path, opens a native desktop form (Fyne), and on submit writes answers back into the same file and prints them as JSON on stdout.
- Answers are written by **splicing text**, not re-serializing: `interrogate` owns only the `answer:` and `comment:` keys of each question and leaves every other byte — comments, spacing, key order — untouched.
...
- Add an **`interrogate` skill** that an opsx agent invokes: it authors the questionnaire into `openspec/changes/<change>/questionnaires/<slug>.yaml`, runs the binary in the background, and reads the answers back when it exits.
...
- `interrogate-cli`: the command-line contract and desktop form behaviour — argument handling, rendering, validation, close-without-submit semantics, in-place answer writing, stdout output, and exit codes.
- `interrogate-skill`: the agent-facing contract — when to reach for a questionnaire, where the file goes, how the binary is launched and awaited, and how answers are consumed.
...
- Distribution: `go install`, so the binary is on `PATH` as `interrogate`; `go run .` during development.
