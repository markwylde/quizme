...
- `quizme-skill`: the skill's availability requirement gains a defined distribution and update path, rather than only describing what to do when the command is missing.

## Impact

- `skills/quizme/SKILL.md` becomes the authoritative copy; `.claude/skills/quizme/` becomes generated and git-ignored.
- README gains install, update, and remove instructions.
- The repository needs a GitHub remote before `npx skills add markwylde/quizme` resolves for anyone else.
...
- Bundling the `quizme` binary. The skill travels; the binary is still `go install`, and the skill already handles the case where it is missing.
