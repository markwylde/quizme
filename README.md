...
`Quizme.app`, with `icon.svg` drawn into it — and it runs the same way:
...
```bash
quizme --validate openspec/changes/add-dark-mode/questionnaires/scope.yaml
```
...
inline note beside `id`, the quoting, the indentation. `quizme` owns four
...
The `quizme` skill covers when to use a questionnaire rather than asking in
conversation, where to put the file, and how to act on each outcome.

### Installing the skill

```bash
npx skills add markwylde/quizme --skill quizme
```
...
Name `--skill quizme`: without it the installer offers every skill it finds
here, and this repository also carries its own OpenSpec tooling under
`.claude/`.

```bash
npx skills update quizme     # pull a newer version
npx skills remove quizme     # take it out again
```
...
`skills/quizme/SKILL.md` is the only committed copy, and the one the
installer reads. The copy under `.claude/skills/quizme/` is generated by
...
make skill                                  # regenerate .claude/skills/quizme/

QUIZME_RENDER=1 go test ./internal/ui/ -run RenderPreview   # refresh the screenshots
```
...
```bash
make bundle
open -a ./Quizme.app --args examples/demo.yaml
```
